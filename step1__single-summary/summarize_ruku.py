...
def call_gemini_api(api_key, model, prompt):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # If 429 rate limit, wait at least 30 seconds to let the rate limit reset
            if e.code == 429:
...
def main():
    ...
    api_key = (
        env.get("GEMINI_API_KEY_STEP1") or 
        env.get("GEMINI_API_KEY") or 
        os.environ.get("GEMINI_API_KEY_STEP1") or 
        os.environ.get("GEMINI_API_KEY")
    )
    
    if not api_key:
        print("Error: GEMINI_API_KEY_STEP1 or GEMINI_API_KEY not found in .env or environment variables.", file=sys.stderr)
        sys.exit(1)
    ...
    for entry in todo_list:
        ...
        for src_key in active_sources:
            ...
            # Call API
            response_text = call_gemini_api(api_key, GEMINI_MODEL, prompt)
