...
Your task is to take a Ruku block script written in Roman Urdu and convert it into a structured "Visual Directions Sheet" in JSON format, split by scenes.
...
4. **English Descriptions**: All visual descriptions and remarks must be written in English. The script field must preserve the original language (Roman Urdu).
"""
...
def call_gemini_api(api_key, model, system_prompt, user_content, response_schema):
...
def process_track(api_key, script_dir, root_dir, limit, ruku_filter, force_flag, delay, max_tafseer_scenes):
    print("\n===== Starting Step 4 (ROMAN URDU Track - Combined) =====")
    todo_path = os.path.join(script_dir, "guiding_resources", "todo_visuals_urdu.json")
    ...
    for entry in todo_list:
        ...
        print(f"\n>>> [ROMAN URDU] Generating Visual & Subdivided Sheets for Ruku {abs_ruku} (Surah {surah_num:03d} {surah_name}, Relative Ruku {rel_ruku})")
...
def main():
    parser = argparse.ArgumentParser(description="Generate scene-by-scene Roman Urdu visual directions and subdivide them into phase blocks.")
    ...
    api_key = (
        env.get("GEMINI_API_KEY_STEP4") or 
        env.get("GEMINI_API_KEY") or 
        os.environ.get("GEMINI_API_KEY_STEP4") or 
        os.environ.get("GEMINI_API_KEY")
    )
    
    if not api_key:
        print("Error: GEMINI_API_KEY_STEP4 or GEMINI_API_KEY not found in .env or environment variables.", file=sys.stderr)
        sys.exit(1)
        
    process_track(api_key, script_dir, root_dir, args.limit, args.ruku, args.force, args.delay, args.max_tafseer_scenes)
    print("\nRoman Urdu combined Step 4 and 5 processing finished.")
