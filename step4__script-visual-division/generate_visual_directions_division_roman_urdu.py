...
Your task is to take a Ruku block script written in Roman Urdu and convert it into:
...
- All visual descriptions, titles, and remarks must be written in English. The script field must preserve the original language (Roman Urdu).
"""
...
def process_track(script_dir, root_dir, limit, ruku_filter, block_filter, force_flag, delay, max_narrative_scenes):
    print("\n===== Starting Step 4 (ROMAN URDU Track - Visual Groups) =====")
    ...
    todo_list = sync_block_todo_list(todo_path, root_dir, "ur")

    processed_blocks = 0

    for entry in todo_list:
        if limit is not None and processed_blocks >= limit:
            print(f"Reached processing limit of {limit} Blocks for ROMAN URDU track. Stopping.")
            break
        ...
        print(f"\n>>> [ROMAN URDU] Generating Visual Groups for Ruku {abs_ruku} Block {idx} (Surah {surah_num:03d} {surah_name})")
...
def main():
    parser = argparse.ArgumentParser(description="Generate scene-by-scene Roman Urdu visual directions with structured visual groups.")
    ...
    print("\nRoman Urdu Step 4 processing finished.")
